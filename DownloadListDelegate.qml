import QtQuick 2.0
import "./ProsaicDefine.js" as Prosaic
...
Rectangle {
	...
	Text {
		color: "#868686"
		text: url + "\n" + bytesToHumanReadable(downloadedBytes) + " of " + bytesToHumanReadable(totalBytes)
		...
		onClicked: {
			if ( active ) {
				if ( downloadStatus == Prosaic.DIS.INACTIVE) {
					...
					active = false
					humbleDownloader.changeDownloadItemState(ident, Prosaic.DIS.ACTIVE )
				} else if ( downloadStatus == Prosaic.DIS.ACTIVE) {
					//downloadStatus = Prosaic.DIS.UNKNOWN
					active = false
					humbleDownloader.changeDownloadItemState(ident, Prosaic.DIS.INACTIVE )
				}
			}
		}
	}
	...
	ActionButton {
		...
		onClicked: {
			humbleDownloader.openDirectory(ident)
		}
	}
